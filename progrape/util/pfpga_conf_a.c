...
void config_pfpga_a(int devid, char* fname, int chipid)
{
...
int main(int argc, char* argv[])
{
  ...
  if (argc < 2) {
    fprintf(stderr, "%s <bitfile> <chipid> <devid>\n", argv[0]);
    exit(-1);
  }
  ...
  if (argv[2] == NULL) {
    ...
    chipid = atoi(argv[2]);
    if (chipid > 3) {
      fprintf(stderr, "%s : invalid argument <chipid> %s:%d\n", argv[0], __FILE__, __LINE__);
      exit(-1);
    }
  }
  ...
  fprintf(stderr, "%s configures a PFPGA%i on progrape%i ...\n", argv[0], chipid, devid);
  ...
  {
    sprintf(pfpga_info, "%s on <PFPGA%d>", argv[1], chipid);
    config_pfpga_a(devid, argv[1], chipid);
